...
def rerank_eval_one_run(query, st_querys, tokenizer, model):
#     print(len(st_querys))
...
class OneRun:
    ...
    def in_topk(self, topk):
        return any(answer in self.gold for answer in getattr(self, 'top'+str(topk)))
...
class EvalSession:
    ...
    def get_topk(self, topks=[1, 3, 5]):
        for run in self.runs:
            positive_prob = rerank_eval_one_run(run.query, run.recall, self.tokenizer, self.model)
            for topk in topks:
